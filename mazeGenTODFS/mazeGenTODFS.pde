...
ArrayList<Node> stack_to_dfs, checked;
...
void setup() {
  ...
  stack_to_dfs = new ArrayList<Node>();
...
void draw() {
  ...
  Node to_check = stack_to_dfs.remove(stack_to_dfs.size() - 1);
  checked.add(to_check);
  
  for (Node neighbor : to_check.final_n) {
    if (!checked.contains(neighbor)) {
      //checked.add(neighbor);
      stack_to_dfs.add(neighbor);
